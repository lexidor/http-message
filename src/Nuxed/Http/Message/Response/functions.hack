...
function must_revalidate(Http\Message\IResponse $response): bool {
  return _Private\has_cache_control_directive($response, 'must-revalidate') ||
    _Private\has_cache_control_directive($response, 'proxy-revalidate');
}
...
function with_immutable(
  ...
  return _Private\with_cache_control_directive($response, 'immutable');
}
...
function without_immutable(
  ...
  return _Private\without_cache_control_directive($response, 'immutable');
}
...
function with_max_age(
  ...
  return _Private\with_cache_control_directive($response, 'max-age', $value);
}
...
function get_max_age(Http\Message\IResponse $response): ?int {
  ...
  if (_Private\has_cache_control_directive($response, 's-maxage')) {
    return _Private\get_cache_control_directive($response, 's-maxage');
  }

  if (_Private\has_cache_control_directive($response, 'max-age')) {
    return _Private\get_cache_control_directive($response, 'max-age');
  }
...
function is_invalid(Http\Message\IResponse $response): bool {
  return $response->getStatusCode() < 100 || $response->getStatusCode() >= 600;
}
...
function is_empty(Http\Message\IResponse $response): bool {
  return C\contains(vec[204, 304], $response->getStatusCode());
}
 ...
 * (302 by default).
 *
 * Note: Http\Message\IResponse function overwrites the `location` $headers value.
 */
function redirect(
  Message\Uri $uri,
  int $status = 302,
